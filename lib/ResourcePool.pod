...
Specifies the maximum concurrent resources managed by this pool. If the limit is reached the L<get()|> method will return undef.

Default: 5

=item B<MaxTry>

Specifies how many dead resources the L<get()|> method checks before it returns undef. Normally the L<get()|> method doesn't return dead resources (e.g. broken connections). In the case there is a broken connection in the pool, the L<get()|> method throws it away and takes the next resource out of the pool. The L<get()|> method tries not more then MaxTry resources before it returns undef.
...
=back
So this call to L<get()|> would have taken about 7 seconds.
...
The number of sleeps can not be more than L<MaxTry|> - 1, if you specify more values the list is truncated. If you specify less values the list is extended using the last value for the extended elements. e.g. [0, 1] in the above example would have been extended to [0, 1, 1, 1]

If you have L<Time::HiRes|Time::HiRes> installed on your system, you can specify a fractal number of seconds.

If you are doing load balancing you should use L<ResourcePool::LoadBalancer's SleepOnFail option|LoadBalancer> instead of this one.

Please see also the L<TIMEOUTS|> section for more information about timeouts.
...
Returns a resource. This resource has to be given back via the L<free()|> or L<fail()|> method. The L<get()|> method calls the L<precheck() method of the according resource|Resource> to determine if a resource is valid. The L<get()|> method may return undef if there is no valid resource available. (e.g. because the L<Max|> or the L<MaxTry|> values are reached)


=head2 S<$pool-E<gt>free($resource)>

Marks a resource as free. This resource will be re-used by L<get()|> calls. The free() method calls the L<postcheck() method of the resource|Resource> to determine if the resource is valid.
...
This section includes a typical configuration for a persistent L<Net::LDAP|Net::LDAP> pool.
 ...
 my $factory =  ResourcePool::Factory::Net::LDAP->new("ldaphost",
                  [ 'cn=Manager,dc=fatalmind,dc=com',
                    password => 'secret' ],
                  [ version => 2 ]
 );
...
If you want to make sure that there is always a connection opened, you can use  PreCreate => 1 (see L<PreCreate|>)

=item Covers server outages

As long as the ldaphost doesn't crash while you are actually using it (between L<get()|> and L<free()|>) the ResourcePool would transparently cover any ldaphost outages which take less then 7 seconds.

You can easily change the time how long ResourcePool tries to create a connection with the L<MaxTry|> and L<SleepOnFail|> options. If you would have used  MaxTry => 1000000 in the example above, the L<get()|> method could have blocked for about 46 days until giving up (after the first 3 tries it would have tried every 4 seconds to establish a connection).

If you have a connection problem with the resource while using it, it's the best practice to give it back to the pool using the L<fail()|> method. In that case the ResourcePool just throws it away. After that you can try again to L<get()|> a new resource.

=item No magic

The resources you obtain from the L<get()|> method are in no way magic. They are just plain L<Net::LDAP|Net::LDAP> handles as you would create them yourself. You still MUST apply all the error handling when using this resource.
...
For this reason ResourcePool leaves you alone as soon as you have obtained a resource until you give it back to the pool (between L<get()|> and L<free()|> or L<fail()|>).

ResourcePool does not magically modify L<DBI|DBI> or L<Net::LDAP|Net::LDAP> to do a transparent reconnect if one connection fails while you are using it. The smartness of ResourcePool lies in the L<get()|> method, therefore you must keep the resources as short as possible outside of the pool and obtain a resource via L<get()|> if you need one.

But you have also to be careful to do too many L<get()|>/ L<free()|> calls since this might also cause a overhead because of the L<precheck()|Resource> and L<postcheck()|Resource> methods.
...
Even if it sounds very invitingly to create a ResourcePool with some L<PreCreated|> connections and afterwards doing the fork, its just not save. Thats for several reasons: 1. There is no locking between the different processes (the same resource could be used more then once simultaneously). 2. There is no guarantee the the underlaying implementations do not keep some state-information locally (this could cause protocol errors). 3. It's impossible to share newly opened connection with the other processes.
...
L<ResourcePool::LoadBalancer|LoadBalancer>,
L<ResourcePool::Resource|Resource>,
L<ResourcePool::Factory|Factory>,
