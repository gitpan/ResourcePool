...
Each ResourcePool::Resource class gets constructed by the corresponding L<ResourcePool::Factory|Factory> class. In fact the Factory classes are just used to store the parameters which are required to construct a resource.

Every class which is derived from ResourcePool::Resource must override the L<close|>, L<fail_close|> and L<get_plain_resource|> methods and should override at least one of L<precheck|> or L<postcheck|>.
...
Checks a connection. This method is called by the L<get() method of the ResourcePool|ResourcePool> before it returns a connection. The default implementation always returns true.

B<Returns:> true if the connection is valid

=head2 S<$resource-E<gt>postcheck>

Checks a connection. This method is called by the L<free() method of the ResourcePool|ResourcePool> to check if a connection is still valid. The default implementation always returns true.
...
L<ResourcePool::Resource::DBI|Resource::DBI>,
L<ResourcePool::Resource::Net::LDAP|Resource::LDAP>
