...
Is a adapter which gives each resource a common interface. This interface covers basic requirements such as closing a resource. L<ResourcePool::Resource|Resource> acts as a base class for all resources.

=item a factory

Is used to construct resources on demand. Is usually not more than a storage of the required information to create a resource. L<ResourcePool::Factory|Factory> acts as a base class for all factories.
...
A resource adapter has to provide a generic interface to L<ResourcePool|ResourcePool> for your resource. This interface is used to interact with the resource. Some of the methods are optional, you can just skip them if you have carefully derived your own resource adapter from L<ResourcePool::Resource|Resource> which will provide some defaults for you.
...
The precheck() method is very important to the high availability functionality of L<ResourcePool|ResourcePool> and L<ResourcePool::LoadBalancer|LoadBalancer>. This method, together with L<postcheck()|>, builds the backbone of the fault detection mechanism. If you want L<ResourcePool|ResourcePool> to detect broken resources you need to carefully implement this method to return a false value if the resource is not longer valid.

This method is most probably implemented by doing so nop (no operation) with the resource. Choose the cheapest non modifying operation possible and analyze it's result. If everything is as expected return true otherwise false. (The L<DBI extension|Resource::DBI> does a ping() to detect broken connections)
...
This method just returns the internally stored resource. You have to return the actual resource you are implementing the adapter for. E.g. the DBI handle or whatever your extension manages. The return value of this method will be passed to the client code as return value of the L<get()|ResourcePool> method.
...
From the moment on where this method returns you lose control over this resource. Later, when you get back the resource from the client (which has called the L<free() method of ResourcePool|ResourcePool>) you can not make any assumptions of the state of the resource. The client could have done everything which can be done with this resource, including closing it (e.g. disconnecting from the database).

=item postcheck

The postcheck() method is very similar to the previously described L<precheck()|> method. The only difference is that the postcheck() method is called after the client has returned a resource to the pool.

As mentioned above, you can not make any assumptions of the state of the resource. The client could have done everything. As for the L<precheck()|> method you can use this method also to place some code which restores the original state or doing some cleanup (e.g. a rollback() on a database connection).
...
This method is used to close a resource which is known to be broken (either because one of L<precheck()|> or L<postcheck()|> failed or the client used the L<fail() method of ResourcePool|ResourcePool> to hand it back to the pool.

The difference to the L<close()|> method is very subtle. For many resource there is no difference anyway. For some others you might want to skip some cleanup operations which will fail anyway if the resource is broken.
...
The L<ResourcePool::Factory|Factory> class acts as base for all factories. Besides the constructor the L<ResourcePool::Factory|Factory> defines only two methods which should be overloaded.
...
Returns a resource adapter (a L<ResourcePool::Resource|Resource> derived class). L<ResourcePool|ResourcePool> uses this method to create new resources as required. As a result of the fact the the constructor of a resource adapter may fail and return undef, the create_resource method is also allowed to return undef. In that case L<ResourcePool|ResourcePool> would apply the error handling as configured.
...
As you have learned every L<ResourcePool|ResourcePool> consists of two parts. The namespace where you should place this two packages are L<ResourcePool::Factory|Factory> and L<ResourcePool::Resource|Resource>. Below this two namespaces you should add the complete perl-class name for which resource your extension is. E.g. The L<Net::LDAP|Net::LDAP> factory and resource adapter are called L<ResourcePool::Factory::Net::LDAP|Factory::LDAP> and L<ResourcePool::Resource::Net::LDAP|Resource::LDAP>.
